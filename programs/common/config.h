...
// inside a few thousand cycles; the default values are tuned for the 25 MHz
// CPU clock on the board, which is CLOCK_50 divided by two because the
// single-cycle core only closes timing at 29 MHz. delay_loop costs roughly two
// cycles per iteration on the single-cycle core, so 12.5 million iterations is
// about one second.
...
// All of these are iteration counts, so every one is half its 50 MHz value
// now that the CPU domain runs at 25 MHz. The wall clock times are unchanged.
#define BLINK_DELAY   1500000u   // about 0.12 s per step
#define COUNTER_DELAY 3000000u   // about 0.24 s per count
#define MIRROR_DELAY  12500u     // about 1 ms, so switches feel instant
#define FIB_DELAY     6000000u   // about 0.5 s per term

#define MEMTEST_WORDS 64u    // the full 256 byte test window

#define REACT_DELAY_MIN   12500000u   // reaction: 1.0 s to about 3.7 s
#define REACT_DELAY_MASK  0x1FFFFFFu
#define REACT_TIMEOUT     50000000u   // roughly 12 s of polling
#define REACT_HOLD        25000000u   // about 2 s of result on the displays
